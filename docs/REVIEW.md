...
| REL-02 | medium | npm@latest (12.x) auto-corrected the manifest at publish time and logged `"bin[llm-jury]" script name dist/cli/bin.js was invalid and removed` (0.2.0 and 0.3.0 publish logs), plus `No bin file found` because the publish job had not built `dist/` yet. Reading npm's `@npmcli/package-json` normalize code shows the message fires whenever the path is normalized (`./dist/cli/bin.js` to `dist/cli/bin.js`) and the entry is kept, so no published version lost its bin; the 0.3.0 registry metadata carries it. The risk was that CI could not tell a cosmetic correction from a real one. | VERIFIED (release run 35982563148 log, npm 12.1.0 source) | fixed (2026-09-24): bin path written without `./`, release builds before publishing, CI dry-runs the publish with npm@latest and fails on any auto-correction |

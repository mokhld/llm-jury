...
| REL-02 | medium | The npm publish job ran `npm publish` on a fresh checkout without `dist/`. npm@latest checks bin targets before `prepack` builds, logs `No bin file found at dist/cli/bin.js`, and says it removed the bin (seen in the 0.2.0 and 0.3.0 publish logs). The published 0.3.0 metadata still carries the bin, but a stricter npm could ship a package without its CLI. CI's pack smoke test used the older bundled npm and could not see it. | VERIFIED (release run 35982563148 log) | fixed (2026-09-24): release builds before publishing; CI dry-runs the publish with npm@latest and fails on any manifest auto-correction |
